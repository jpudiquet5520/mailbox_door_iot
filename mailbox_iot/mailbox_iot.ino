...
void setup_wifi() {
    ...
    while (WiFi.status() != WL_CONNECTED) {
        ...
        delay(25); // Pauses loop for 25ms between each iteration; for stabalization
        
        // Did wifi fail to connect then time out?
        if (millis()- start_time > wifi_timeout ) 
        {
            Serial.println("Wifi Failed to connect ..timeout..");

            //TODO: Uncomment this
            //esp32_sleep(); // Puts the ESP32 into a low-power sleep mode to conserve battery.
        }
    
    }//End while()
 
    rssi= WiFi.RSSI(); // RSSI(Received Signal Strength Indicator); Quality of WiFi Connection
...
void setup() {
    ...
    //TODO: NEED TO COMMENT THESE OUT, FOR TROUBLESHOOTING setup_wifi()
    //setup_wifi();  
